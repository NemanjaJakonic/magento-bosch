    ...
    protected function _prepareForm() 
    {
        ...
        $fieldset->addField(
            ...
            'after_element_html' => $this->__(
                '<small>Please use numbers '
                . 'only between 1-' . $columnsize . ' in this field</small>'
            )
            )
        );
        $fieldset->addField(
            'target_url', 'text', array(
        ...
        $headerCotent = $form->addFieldset(
            'megamenu_headercontent_form', array(
        ...
        foreach ($blockcollection as $block) {
            ...
            $blocks[] = array('value' => $blockId, 'label' => $blocktitle);
        }

        $headerCotent->addField(
            'header', 'select', array(
        ...
        //feature products   
        $featureProduct = $form->addFieldset(
            'megamenu_feature_form', array(
            'legend' => Mage::helper('megamenu')->__('Feature Product')
                )
        );

        $featureProduct->addField(
            'feature_type', 'select', array(
        ...
        $featureProduct->addField(
            'feature_product_ids', 'text', array(
        ...
        $mainCotent->addField(
            'main_content_category_ids', 'text', array(
        ...
        $mainCotent->addField(
            'main_content_product_ids', 'text', array(
        ...
        $mainCotent->addField(
            'content_container', 'editor', array(
